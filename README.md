...
- `Marketplace.SaaS.Client`. It is only used in the landing code flow.
...
Generate URLs to the functions and input them into the technical specification of your Marketplace offer. Ensure the Azure Functions `code` argument is included in the URL, but do not rely on it as the only protection for the webhook endpoint. The course tutorial considers URL secrets an anti-pattern, but does not go through how the webhook can be properly secured otherwise.

## Database
The sample application uses an in-memory database when run locally (no data persists after stopping the application). This can be easily switched to use SQL Server or another database system.

## Testing
- Run unit tests with the command: `dotnet test`.
- The project includes basic tests for the landing and webhook license modification scenarios. The Marketplace SaaS client is mocked, with an example payload provided for reference.
- The webhook test uses a `WebApplicationBuilder` service setup and WireMock to serve OpenID Connect discovery and signing keys for JWT validation.
